"""`ReservationDeposit.payment_method` pasa del catalogo global al del hotel.
...
def migrate_payment_methods(apps, schema_editor):
    Model = apps.get_model("reservations", "ReservationDeposit")
    PaymentMethod = apps.get_model("hotel_settings", "PaymentMethod")

    cache = {}
    for row in Model.objects.select_related("payment_method", "reservation").all():
        legacy = row.payment_method
        hotel_id = getattr(getattr(row, "reservation", None), "hotel_settings_id", None)
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("reservations", "0009_alter_reservation_hotel_settings"),
        ("hotel_settings", "0006_seed_payment_methods"),
    ]

    operations = [
        migrations.AddField(
            model_name="reservationdeposit",
        ...
        migrations.RemoveField(model_name="reservationdeposit", name="payment_method"),
