...
import { PromotionFormPayload, PromotionI } from '../promotion-model';
...
export class UpdatePromotion implements OnInit, OnChanges {
  @Input() promotion: PromotionI | null = null;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['promotion'] && this.promotion) {
      this.applyPromotion(this.promotion);
    }
  }

  /** El alcance no se guarda en el modelo: se deduce de a que apunta la promocion. */
  private applyPromotion(promotion: PromotionI): void {
    const scope: PromotionTargetScope = promotion.service
      ? 'SERVICE'
      : promotion.package
        ? 'PACKAGE'
        : 'GENERAL';
    ...
    this.promotionForm.reset({
      ...
      discount_value: this.toDiscountNumber(promotion.discount_value),
      target_scope: scope,
      service: promotion.service ?? null,
      package: promotion.package ?? null,
    ...
    this.applyTargetValidators(scope);
    this.errorMessage = '';
  }
  ...
  submit(): void {
    ...
    if (!promotionId) {
      this.errorMessage = 'No se encontro la promocion que se quiere editar.';
      return;
    }

    this.saving = true;
    this.promotionsService.updatePromotion(promotionId, payload).subscribe({
      next: () => {
        this.saving = false;
        this.updated.emit();
        this.closeDrawer();
  ...
  private resolveHotelSettingsId(): number | null {
    ...
    const fromPromotion = Number(this.promotion?.hotel_settings);
    if (fromPromotion > 0) return fromPromotion;
  ...
  private extractErrorMessage(error: unknown): string {
    const fallback = 'No se pudo guardar la promocion. Revisa los datos e intenta nuevamente.';
